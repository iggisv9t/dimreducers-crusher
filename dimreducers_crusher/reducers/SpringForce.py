...
class SpringForce(AbstractReducer):
    def __init__(self, d: int = 2, random_state: int = 0, **kwargs):
        super().__init__(d, random_state)
        warnings.warn("Setting random seed does not affect SpringForce.", UserWarning)
        warnings.warn("SpringForce supports only d = 2.", UserWarning)
        self.fitted = None

    def fit_transform(self, x: np.ndarray, **kwargs) -> np.ndarray:
        self.fitted = fl.spring_layout(dataset = x, algorithm = fl.SpringForce)
