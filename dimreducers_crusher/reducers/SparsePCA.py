...
from sklearn.decomposition import SparsePCA as skspca
import numpy as np


class SparsePCA(AbstractReducer):
    def __init__(self, d: int = 2, random_state: int = 0, **kwargs):
        super().__init__(d, random_state)
        self._main = skspca(n_components=d, random_state=random_state, **kwargs)
